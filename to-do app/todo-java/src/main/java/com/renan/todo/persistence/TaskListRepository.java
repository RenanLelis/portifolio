...
//    @Query(nativeQuery = true, value = "SELECT ID, LIST_NAME, LIST_DESCRIPTION FROM TASK_LIST WHERE ID_USER = :idUser")
    ...
    public void updateTaskList(@Param("listName") String listName, @Param("listDescription") String listDescription, @Param("idList") Integer idList, @Param("idUser") Integer idUser);

    /**
     * Dele the list from the database
     *
     * @param idList - list id
     * @param idUser - user id
     */
    ...
    //    @Query(nativeQuery = true, value = "DELETE t.*, tl.* FROM TASK t LEFT JOIN TASK_LIST tl ON tl.ID = t.ID_LIST WHERE tl.ID = :idList AND tl.ID_USER = :idUser")

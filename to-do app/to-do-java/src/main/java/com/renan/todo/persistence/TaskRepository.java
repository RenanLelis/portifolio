...
public interface TaskRepository extends JpaRepository<Task, Integer> {
	 ...
	 */
	@Query(value = "SELECT t.id, t.taskName, t.taskDescription, t.deadline, t.taskStatus, t.list FROM Task t WHERE t.user.id = :idUser")
	...
	public void moveTasksToDefaultList(@Param("idTaskListOrigin") Integer idTaskListOrigin,
	...
	public void moveTasksToAnotherList(@Param("idTaskListOrigin") Integer idTaskListOrigin,
	        @Param("idTaskListDestiny") Integer idTaskListDestiny, @Param("idUser") Integer idUser);

	/**
	 * change the list for the task
	 * 
	 * @param id        - task id
	 * @param idNewList - list id
	 * @param idUser    - user id
	 */
	@Transactional
	@Modifying
	@Query(nativeQuery = true, value = "UPDATE TASK SET ID_LIST = :idNewList WHERE ID = :id AND ID_USER = :idUser")
	public void moveTaskForList(@Param("id") Integer id, @Param("idNewList") Integer idNewList,
	        @Param("idUser") Integer idUser);
	...
	public void updateTask(@Param("id") Integer id, @Param("taskName") String taskName,
	        @Param("taskDescription") String taskDescription, @Param("deadline") String deadline,
	        @Param("idUser") Integer idUser);
	...
	@Transactional
	@Modifying
	@Query(value = "UPDATE Task t set t.taskStatus = :status where t.id = :id and t.user.id = :idUser")
	public void updateStatusTask(@Param("id") Integer id, @Param("status") Integer status,
	        @Param("idUser") Integer idUser);

	/**
	 * Update the status for all the tasks from the list
	 *
	 * @param idList - list id
	 * @param status - new status
	 * @param idUser - user id
	...
	@Transactional
	@Modifying
	@Query(value = "UPDATE Task t set t.taskStatus = :status where t.list.id = :idList and t.user.id = :idUser")
	public void UpdateStatusTasksFromList(@Param("idList") Integer idList, @Param("status") Integer status,
	        @Param("idUser") Integer idUser);
